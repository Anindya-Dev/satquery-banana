  ...
  const handleRunAnalysis = async () => {
    ...
    try {
      const response = await fetch("http://localhost:8000/api/v1/analyze", {
        ...
        body: JSON.stringify({
          query: queryText,
          task_type: currentTask === "SINGLE_IMAGE_VQA" ? "Visual Question Answering" :
                     currentTask === "VISUAL_GROUNDING" ? "Visual Grounding & Segmentation" :
                     currentTask === "CHANGE_DETECTION" ? "Bi-Temporal Change Detection" :
                     currentTask === "OPTICAL_SAR_FUSION" ? "Optical-SAR Multimodal Fusion" :
                     "Auto-Classified Query"
        })
      });
      ...
      setPipelineToast(`API Success! Execution Time: ${data.processing_time_ms}ms | Confidence: ${data.confidence?.rating}`);

      // Update active scenario with live API results
      setActiveScenario(prev => ({
        ...prev,
        answer: data.summary_answer,
        confidence: Math.round((data.confidence?.score || 0.95) * 100),
        evidenceChain: data.evidence_chain.map(ev => ({
          id: ev.evidence_id,
          source: ev.layer,
          proof: `${ev.description} [${ev.metric_name}: ${ev.metric_value} ${ev.unit}]`
        }))
      }));

    } catch (err) {
      console.warn("Backend API offline or unreachable, falling back to deterministic local state:", err);
      setPipelineToast("Pipeline Complete (Local Deterministic State).");
    } finally {
      setTimeout(() => setIsAnalyzing(false), 800);
      setTimeout(() => setPipelineToast(null), 3500);
    }
  };
