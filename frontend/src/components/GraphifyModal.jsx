...
export default function GraphifyModal({ isOpen, onClose, scenarioData }) {
  ...
  // Fetch or construct Graphify JSON topology
  useEffect(() => {
    if (!isOpen) return;

    // Use default scenario or fallback analysis dict
    const payload = scenarioData || {
      ...
      candidate_tiles: [
        ...
        {
          tile_id: "tile-s1-sar-kolkata-02",
          sensor: "Sentinel-1 SAR C-Band",
          acquisition_time: "2024-05-21",
          cloud_cover: 0.0,
          metrics: { sigma0_db: -14.8, lee_speckle_filtered: true }
        }
      ],
      evidence_chain: [
        {
        ...
        {
          evidence_id: "ev-902",
          description: "Sentinel-1 SAR VV backscatter (-14.8 dB) confirms open water surface through cloud layer."
        }
      ],
      validated_claims: [
        {
    ...
    fetch('http://localhost:8000/api/v1/graphify', {
      method: 'POST',
      ...
      .catch(() => {
        // Mock fallback topology generator if backend offline
  ...
  // Calculate layout positions for nodes in a clean semi-circular / radial topology
  const calculateInitialPositions = (nodes) => {
    ...
    // Layout hierarchy rows
    queryNodes.forEach((n, i) => { pos[n.id] = { x: center.x, y: 70 }; });
    ...
    confNodes.forEach((n, i) => { pos[n.id] = { x: center.x, y: 560 }; });
